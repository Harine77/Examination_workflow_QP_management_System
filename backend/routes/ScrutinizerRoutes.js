...
router.get('/papers/:id/verify-mapping', async (req, res) => {
  try {
    ...
    for (const q of questions) {
      ...
      const assignedCO = q.CourseOutcome?.coNumber || null;
  ...
  try {
    ...
    if (!['with_scrutinizer1', 'submitted'].includes(paper.status)) {
      return res.status(400).json({
        success: false,
        error: `Paper must be at scrutinizer1 stage. Current: '${paper.status}'`
      });
    }
...
router.post('/randomize/:courseId', async (req, res) => {
  ...
  try {
    const papers = await QuestionPaper.findAll({
      ...
      include: [{ model: Question }, { model: Course, attributes: ['id', 'courseCode', 'courseName'] }],
    });

    if (papers.length < 2) {
      return res.status(400).json({
        success: false,
        error: `Need at least 2 papers with status 'with_scrutinizer2' for this course. Found: ${papers.length}`
      });
    }
    ...
    const pickedQuestions = Object.entries(slotMap).map(([, candidates]) => {
    ...
    await QuestionPaper.update({ status: 'randomized' }, { where: { id: papers.map(p => p.id) } });
